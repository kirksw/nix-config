  ...
  hooks = [
    ...
    }
    {
      ...
      command = ''
        ...
        LIFEOS_EVENT_FILE="$EVENT_FILE" LIFEOS_SESSION_FILE="$SESSION_FILE" python3 <<'PY'
import json
import os
import re
...
lifeos = Path("/Users/kisw/git/github.com/kirksw/lifeOS")
session_file = Path(os.environ.get("LIFEOS_SESSION_FILE", ""))
if not lifeos.exists() or not session_file.exists():
    raise SystemExit(0)
...
try:
    event_file = Path(os.environ.get("LIFEOS_EVENT_FILE", ""))
...
workspace = lifeos / "workspaces" / scope
