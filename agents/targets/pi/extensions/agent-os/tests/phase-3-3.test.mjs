...
test("task state changes refresh only the owning thread generated block", async () => {
  ...
  const created = await createTask(workspace, "thread-a", "Ship views");
  const active = { thread: "thread-a", task: created.path };
  const agentos = context(workspace, "Thread", "thread-a");

  await handleTask(`status ${created.id} specced`, agentos, active, () => {});
  let text = await fs.readFile(readme, "utf8");
  assert.match(text, /Human thread notes must stay/);
  assert.match(text, /Active task: ship-views/);
  ...
  await handleTask(`run ${created.id}`, agentos, active, () => {}, async () => {});
  await handleTask(`run-report ${created.id} success Checks passed`, agentos, active, () => {});
...
test("render preserves human text, canonical markers, and all three generated levels", async () => {
  ...
  const created = await createTask(workspace, "thread-a", "Ship views");
  await transitionTask(workspace, "thread-a", created.id, "specced");
  ...
  await handleRender("", context(workspace), { task: created.path });
  await handleRender("", context(workspace), { task: created.path });
  ...
  assert.match(files[1], /Tasks: ship-views \(specced\)/);
  assert.match(files[1], /Views shipped \(in_progress\)/);
  assert.match(files[2], /Keep this tracker note/);
  assert.match(files[2], /tasks: ship-views \(specced\)/);
});
