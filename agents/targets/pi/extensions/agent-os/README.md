...
Native Pi commands for the git-backed Agent OS workspace. Markdown thread, workpackage, and run files remain the source of truth; JSONL is only the runtime store and mailbox transport.
...
- **Thread** — a thread is selected without a workpackage.
- **Factory** — a thread and workpackage are selected.

`/agent-os status` is visible in every mode and reports mode, thread, workpackage, and unread mailbox count.
...
```sh
agent-os launch --thread <slug> --workpackage <id-or-path> --project /path/to/project
```
...
Todos route to `workspace/inbox/todos.md` in OS mode, an existing thread `todos.md` (otherwise the thread README manual area) in Thread mode, and the selected workpackage file in Factory mode. Existing frontmatter and human-authored text are preserved.
...
- **ThreadOS** (`Thread` mode) can read only `workspace/threads/<thread>/`. It may write thread-owned files, including `package.md`, `input/**`, the thread README, and thread notes. Factory-owned `runs/**` and `output/**` are read-only.
- **FactoryOS** (`Factory` mode) can read only the selected workpackage bundle. It may write only `runs/**` and `output/**`; `package.md` and `input/**` are read-only ThreadOS inputs.
...
- The extension also injects the active policy into the agent system prompt. A binding cannot switch its thread or workpackage after launch.

Canonical workpackage layout:

```text
workspace/threads/<thread>/workpackages/<wp>/
  package.md       # ThreadOS
  input/           # ThreadOS
  runs/            # FactoryOS
  output/          # FactoryOS
