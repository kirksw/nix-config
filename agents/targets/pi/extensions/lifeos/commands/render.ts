...
export async function handleRender(_args: string, lifeos: LifeOsContext): Promise<string> {
  ...
  for (const thread of data.threads) {
    results.push(await renderThreadReadme(lifeos.workspacePath, thread, data.blockers, data.decisions));
  }
