...
import { THREAD_KINDS, type ThreadKind, type ThreadRecord } from "../core/schema.js";
...
function parseKind(args: string): { title: string; kind: ThreadKind } {
  ...
  if (!THREAD_KINDS.includes(rawKind as ThreadKind)) {
    throw new Error(`invalid kind '${rawKind}' (expected ${THREAD_KINDS.join(" | ")})`);
  }
  const title = args.replace(/(?:^|\s)--kind\s+\S+/, " ").trim().replace(/^['"]|['"]$/g, "");
  if (!title) throw new Error("usage: /lifeos new-thread <title> --kind <kind>");
  return { title, kind: rawKind as ThreadKind };
}

async function uniqueSlug(storePath: string, title: string): Promise<string> {
  const base = slugify(title);
  const existing = new Set((await readJsonl<ThreadRecord>(storePath, "threads")).map((t) => t.slug));
...
export async function handleNewThread(args: string, lifeos: LifeOsContext, setActive: ActiveThreadSetter): Promise<string> {
  ...
  await fs.mkdir(path.join(lifeos.workspacePath, thread.path, "artifacts"), { recursive: true });
  await renderThreadReadme(lifeos.workspacePath, thread, [], []);
  setActive(lifeos.workspacePath, slug);

  return [`# LifeOS thread created`, "", `- ${thread.title}`, `- Slug: ${thread.slug}`, `- Path: ${thread.path}`].join("\n");
}

export async function handleThread(args: string, lifeos: LifeOsContext, setActive: ActiveThreadSetter): Promise<string> {
  requireWritable(lifeos);
  const slug = args.trim();
  if (!slug) throw new Error("usage: /lifeos thread <slug>");
  ...
  setActive(lifeos.workspacePath, slug);
  return [`# LifeOS active thread`, "", `- ${thread.title}`, `- Slug: ${thread.slug}`, `- Status: ${thread.status}`, `- Stage: ${thread.stage}`].join("\n");
}
