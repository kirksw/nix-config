...
import { selectActionableTasks, selectUnfinishedBlockers, selectVisibleTasks } from "./state/selectors.js";
import {
...
function taskDependencyState(state: LiveTaskState, task: (typeof state.tasks)[number]) {
	return (task.blockedBy ?? []).map((id) => ({
		id,
		status: state.tasks.find((candidate) => candidate.id === id)?.status ?? "missing",
	}));
}

/** JSON keeps task text data, rather than allowing it to become instructions. */
export function buildLiveTaskBlock(state: LiveTaskState): string | undefined {
	const openTasks = selectVisibleTasks(state).filter(
		(task) => task.status === "pending" || task.status === "in_progress",
	);
	if (openTasks.length === 0) return undefined;
	const next = selectActionableTasks(state)[0];
	return [
		...
		JSON.stringify({
			tasks: openTasks.map((task) => ({
				...
				status: task.status,
				dependencies: taskDependencyState(state, task),
				unfinishedBlockers: selectUnfinishedBlockers(state, task),
			})),
			nextActionableTask: next ? { id: next.id, subject: next.subject, status: next.status } : null,
		}),
	].join("\n");
}
...
export default function (pi: ExtensionAPI, importOverlay: TodoOverlayImporter = () => import("./todo-overlay.js")) {
	...
	// `input` is the source-level distinction Pi exposes for external user turns;
	// extension messages do not re-arm this guard.
	pi.on("input", (event, ctx) => {
		const id = sid(ctx);
		if ((event.source !== "interactive" && event.source !== "rpc") || id !== getActiveRenderSession()) return;
		const guard = continuationGuards.get(id) ?? closedContinuationGuard();
		continuationGuards.set(id, guard);
	...
	pi.on("agent_settled", (_event, ctx) => {
		...
		if (
			...
			return;
		const next = selectActionableTasks(getState(id))[0];
		if (!next) return;
		// Set before sendMessage so this continuation can never re-arm itself.
		guard.continuationSent = true;
		pi.sendMessage(
			{
				customType: AUTO_CONTINUATION_CUSTOM_TYPE,
				content: "Continue executing the next actionable todo under the todo execution policy.",
				display: false,
			},
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			id = sid(ctx);
			continuationGuards.set(id, closedContinuationGuard());
	...
	pi.on("session_shutdown", async (_event, ctx) => {
		...
		try {
			...
			// A stale child/replaced-session ctx cannot identify a session. It must
			// not evict state or tear down the live foreground.
			if (isStaleCtxError(e)) return;
			throw e;
		}
		// Empty/unknown ids have no session to evict or tear down.
		if (!s) return;
		evictSession(s);
		continuationGuards.delete(s);
		if (s !== getActiveRenderSession()) return;
		...
		// ui proxy, which can throw. The slot is already evicted, so leaving
		// `activeRenderSession` pointing at it would render a fresh EMPTY_STATE.
		// try/finally guarantees pointer-clear + overlay-drop run regardless.
		try {
			todoOverlay?.dispose();
		} finally {
