...
## Local fork notes (nix-config)

- Baseline: exact MIT-licensed `@juicesharp/rpiv-todo` **v2.2.0**; local production changes are limited to prompt-aware execution/context/guard helpers (`index.ts`, `selectors.ts`).
- Local behavior: immutable todo execution policy in `before_agent_start`; fresh hidden untrusted-data live open-task context before every LLM call; at most one guarded follow-up per interactive/RPC user turn when foreground actionable work remains; Pi 0.83 cancellation and child/stale session isolation.
- Verification (this package): `npm install --ignore-scripts --no-package-lock`, `npm test`, `npm run typecheck`.
- Current evidence: 237 tests and a GPT-5.6-luna smoke pass.
- Upstream sync guidance: diff against `@juicesharp/rpiv-todo@2.2.0` first and preserve LICENSE/provenance.
