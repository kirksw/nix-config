...
/** Open tasks whose dependencies are all completed or deleted. */
export function selectActionableTasks(state: TaskState): readonly Task[] {
	return selectVisibleTasks(state).filter(
		(task) =>
			ACTIVE_STATUSES.has(task.status) &&
			(task.blockedBy ?? []).every((dependencyId) => {
...
/** Open blockers for a task, including missing dependencies from replayed state. */
export function selectUnfinishedBlockers(state: TaskState, task: Task): readonly number[] {
	return (task.blockedBy ?? []).filter((dependencyId) => {
		const dependency = state.tasks.find((candidate) => candidate.id === dependencyId);
		return dependency?.status !== "completed" && dependency?.status !== "deleted";
	});
}
