...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "herdr",
		label: "herdr",
		description: "Orchestrate long-running processes and coding agents across Herdr workspaces, tabs, and panes.",
		promptGuidelines: [
			"Use `herdr` for long-running or parallel work in other panes; create a pane first, give it a reusable alias, and preserve the current UI focus by default.",
			"Submit commands atomically with `run`; reserve `send` for raw text or keys. Pane actions target pane aliases or ids, not tab ids.",
			"Use `watch` or `read` for process output and `wait_agent` only for recognized coding-agent panes; use `recent-unwrapped` when soft wrapping would hinder matching.",
			"When a new agent pane's model matters, specify `--model` or ask the user which provider and model to use.",
		],
