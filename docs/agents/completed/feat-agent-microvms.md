...
3. Allocate each VM 2 vCPU, 4096 MB RAM, and a 20 GB persistent `/srv/assistant` volume.
4. Use QEMU user networking and Tailscale inside each guest for remote access.
5. Add SOPS-backed Tailscale auth-key placeholders and mount only each decrypted auth-key directory into its corresponding VM.
6. Record intended KB repo URLs in `/etc/assistant/kb-repo` and `/etc/assistant/profile.json` without cloning repos or adding credentials.

### Risks

- Placeholder Tailscale auth keys must be replaced before activation, otherwise guest Tailscale authentication will fail.
...
- Added SOPS-encrypted placeholder Tailscale auth keys for each VM.
...
- Replace placeholder Tailscale auth keys before applying `nixos-ry4a`.
